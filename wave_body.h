...
typedef struct wav_body: public wav_header
{
    ...
    std::vector<int> monoChannel_sounData; // Holds data for first channel (everything if audio is mono)
    std::vector<int> steroChannel_soundData; // Holds data for second channel (only used when stero)
